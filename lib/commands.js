                ...
                switch(args.length) {
                    ...
                    case 0: // No parameters
                        let result = Object.entries(commands)
                            .map(([_, data]) => `▫️\`${config.command.prefix}${data.usages[0]}\` ${data.description}\n\n`)
                    ...
                    await channel.send(
                        new discord.MessageEmbed()
                            ...
                            .addFields(
                                value.map(day => {
                                    return {
                                        name: `${day.date.day.getMonth()}/${day.date.day.getDate()} ${DateUtil.weekdays[day.date.day.getDay()]} (${day.calorie})`,
                                        value: day.menu,
                                        inline: true
                                    }
                                })
        ...
        execute: (message, args, neisClient) => {
            return new Promise(async (res, rej) => {
                ...
                */
                ...
                await channel.send('', new discord.MessageAttachment(image.toBuffer(), 'test.png'));
            })
        }
    }
